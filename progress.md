...
- [x] **T50** — `linx` — a **lynx/links-style text-mode** browser (freestanding ELF). Fetches HTTP **and HTTPS** via the userspace `web_fetch` layer (the same `web.c`/`tls.c` mxweb/wget use — T51 already landed), then reflows the page to the terminal: whitespace-collapsed word wrap, ANSI styling, inline numbered **`[N]`** link markers, and a lynx-style command prompt (a number follows a link, `b` back, `r` reload, a URL goes, `q` quit, `h` help). Reads input the `bgetline` way so it works in a text VT and under mxterm; also opens local/`file://` paths and has a `--dump` mode. **Reuses mxweb's HTML engine** via the new shared **`html.{h,c}`** core (HTML-entity decode, attribute extraction, URL split/resolve/normalise) — the renderer-independent tokeniser primitives, now shared by both browsers instead of copied. (A literal GNU lynx/links port needs ncurses + a hosted libc/OpenSSL — deferred to the self-hosted toolchain.)
...
- [x] **T57 — centralised Settings app (`mxsettings`).** A macOS-System-Settings-style app — left **sidebar** of categories + right **content pane** — that **reimplements each panel inline** rather than shelling out to the single-purpose apps. Six panels: **Appearance** (wallpaper path → `~/.mxrc Wallpaper=`, applied live by the WM's existing key-poll), **Display** (resolution via `sys_setmode` + the 15 s confirm/auto-revert ported from `mxdisplay`), **Status Bar** (the five `Tray*` toggles → `mxrc_set_int` + a new **`MX_RELOAD_PREFS`** makx message so the dock re-reads + repaints live), **Network** (DHCP renew/release/flush + **static IP/netmask/gateway/DNS** via the new `SYS_NET_CONFIG`), **Date & Time** (live UTC clock + editable Y/M/D H:M:S → the new `SYS_SETTIME`, a CMOS-RTC write), **Autostart** (per-app toggles backing a `~/.mxrc Autostart=` list that `wm.c` launches at desktop startup). New widget **`ui_toggle`** (iOS-style switch) in `gui_ui`. Two new bounded kernel syscalls: **`SYS_SETTIME`** (285, `rtc_write`) and **`SYS_NET_CONFIG`** (286, `net_lwip_config`) — ktested (`settime` in the main gate; `net_config` round-trip in the net section). Gear desktop icon + `18-settings.desktop`. `mxdisplay`/`mxnet` and the dock tray right-click stay (subsumed later). See `docs/gui.md`.
- [x] **T58 — desktop polish (test-feedback batch).** Follow-ups from testing T57/T50:
  - **Live-CD writable home.** On a live-CD boot the elected rootfs is the read-only ISO9660, so writes to `/root/.mxrc` (tray widgets, wallpaper, autostart, `~/.mxwebrc`) failed silently and Settings never took effect. Ubuntu-live style: when `/` is read-only, `vfs_auto_mount` overlays **tmpfs at `/root`, `/home`, `/home/user`** so home writes land in RAM for the session (persist on an installed ext2/FAT32 root). **tmpfs is now multi-instance** — each op takes a namespace id (the mount's `drive` field; `/tmp` stays ns 0) so the home overlays never alias `/tmp` or each other.
  - **Standard `gui_ui` form controls.** Added **`ui_spinner`** (numeric field + up/down steppers), **`ui_radio`**, **`ui_checkbox`**, **`ui_progress`**, **`ui_separator`** — joining button/slider/textbox/label/listbox/vscroll/toggle/menubar. Settings → Date & Time now uses six fixed-pitch spinners (the variable-width text boxes didn't line up with their YYYY/MM/DD labels) + a **Browse…** button on Appearance.
  - **`linx` → full-screen.** Reworked from a prompt-pager into a real links/w3m-style full-screen browser (cell-API): top menu bar, scrollable page with a navigable highlighted **link cursor** (arrows/Tab/Enter), `g` go-URL, `b` back, `r` reload, `F9` File menu, `q` quit. Reuses `html.{h,c}` + `web_fetch`; raw keys via `tkey.h` so it runs in a text VT and under mxterm. `linx --dump` still prints for piping.
  - **Windowed open/save dialog (makx multi-window).** The open/save dialog is now its own centred window, not an in-window overlay: new **`MX_F_DIALOG`** flag + **`mx_open_window()`** let a client own a second window; the WM frees it on a per-window `MX_BYE` (gated to dialog windows so normal apps still reap cleanly). **`br_dialog_window()`** drives it; `br_dialog` gained an **icon/list view toggle**. Rolled out to mxedit / mximg / mxdoom / mxsettings. See `docs/gui.md`.
- [x] **T59 — editing-keys + tray/widget polish (2nd test-feedback batch).**
  - **Navigation keys everywhere.** The PS/2 driver decoded Home/End/Delete but `translate_make` dropped them; now they map to new **`KEY_HOME`/`KEY_END`/`KEY_DELETE`** sentinels. `ui_textbox` gained a **full caret model** (Left/Right/Home/End/Delete/click-to-position + horizontal scroll), so every GUI text field edits properly. Wired Home/End/Delete/PgUp/PgDn into **mxedit**, the **sh** readline (the GUI terminal), **mxweb** (in-page form fields get a caret; page scroll gains PgUp/PgDn/Home/End; arrows stop scrolling while the URL bar is focused), and **linx** (arrows now drive link navigation, the Go-to-URL prompt is editable).
  - **Tray.** The combined CPU/RAM widget split into two independently-toggleable widgets (`TrayCpu`/`TrayRam`); all tray text uses one uniform colour.
  - **Settings.** Date & Time spinner labels re-centred under their value fields; standalone Display + Network desktop icons removed (Settings subsumes them).
  - **Not done (noted):** linx Alt-to-open-menu (Alt+letter isn't delivered to apps — needs kernel Alt-combo support; menu opens via F9/m).
- [x] **T60 — desktop menu bar + RCCM, linx form typing (final GUI batch before the kernel-separation pivot).**
  - **macOS-style desktop chrome (`wm.c`).** The top menu bar gained working **Edit** (Cut / Copy / Paste) and **View** (Auto Arrange) dropdowns, and a **right-click context menu (RCCM)** on the empty desktop offers the same actions (Cut/Copy/Paste · Auto Arrange — *no undo/redo*, per request). Cut/Copy/Paste inject the `^X`/`^C`/`^V` byte into the focused client (so the app's own clipboard handling runs, exactly as if typed); Auto Arrange re-grids every desktop icon via `icon_grid_pos` and persists the new positions. Edit verbs grey out when no window is focused (Finder-style). One shared popup module (`menu_items`/`menu_box`/`draw_desk_menu`/`desk_menu_click`) mirrors the existing dock tray-menu pattern; menu-bar clicks open via `menubar_hit`, empty-desktop right-clicks open the RCCM (gated off windows/icons/dock).
  - **linx form-field typing.** `linx` now renders text-like `<input>`s as editable boxes and captures the enclosing `<form action>` + hidden inputs. A unified **focus ring** merges links **and** fields in document order (Tab/↑/↓/`n`/`p` walk it); pressing keys while a field is focused edits its value with a full caret (Left/Right/Home/End/Delete/Backspace + h-scroll), **Enter submits** the form (`action?name=value&…`, urlencoded, GET-style — mirrors mxweb's `form_submit`), and Tab/Esc leaves the field. Single-letter commands (`q`/`g`/`b`/`r`…) are suppressed while editing so they type literally. `linx --dump` unchanged.
- [x] **T61 — Windows-style desktop icons + a dedicated GUI CI gate (last GUI batch).**
  - **Icon grid (`wm.c`).** Rewrote `icon_grid_pos` from a fixed 2-column row-major layout into a **column-major, height-derived grid** (Windows-style: fill top-to-bottom down a column, wrap to the next; row count from the live desktop height so the bottom row is never clipped — more icons just add a column). New `icon_clamp` keeps every icon on-screen (so a position saved at a larger resolution can't strand one below the dock).
  - **Per-user position persistence in `~/.mxrc`** (not the `.desktop` file, Windows-style): `icon_save_pos`/`icon_load_pos` store `IconPos.<Name>=x,y` so a moved icon's spot survives reboots on **installed** systems and the session on the **live ISO** (the tmpfs home overlay). Dragged drops + Auto Arrange both persist here.
  - **Alphabetical order.** Desktop icons now sort **case-folded by Name** (the Windows default), not by `.desktop` filename — and the shortcut files were renamed to drop their `NN-` numeric prefixes (`00-terminal.desktop` → `terminal.desktop`, …) with the hardcoded `X-Makar-IconX/Y` lines removed. **Auto Arrange** re-grids A→Z (rank-based, no array reorder) and saves the new spots.
  - **GUI tests get their own CI section.** Pulled the headless GUI self-tests out of `shell-smoke.sh` into a dedicated **`gui-smoke`** suite (`/src/userspace/gui-smoke.sh`, marker `GUI-SMOKE`), added a new **`gui.elf desktest`** covering the icon-grid math / on-screen clamp / alphabetical sort / `~/.mxrc` key+parse + a save→load round-trip, and wired a kernel `test=gui-smoke` token + `./run.sh guismoke` target + a **`gui-headless` CI job** (separate from the kernel `ktest` job and the `guitest` boot-screendump job). Deterministic, framebuffer-free, no exec-heavy tcc/libc drivers — so it stays non-flaky. `iso test` still runs all four suites (incore/libc-tcc/shell-smoke/gui-smoke). Also promoted **`kbtest`** (in-guest keyboard injection) to its own CI job — deterministic (asserts on `KBTEST: ALL PASS`, no host typing), with a single auto-retry reserved for the documented `exec: missing params` arg race; CI now covers the keyboard pipeline for the first time.
...
- [x] **T55 — terminal-app mxterm-compliance audit.** Every TUI app works both in a text VT (kernel line discipline) AND under mxterm (raw pipe: char-by-char, no echo, Enter mapped to `\n`). Line-oriented apps (`basic`, `fdisk`) use the **`bgetline`** pattern (raw-detect on the first read → self-echo + Backspace only when raw). Key-driven apps (`cfdisk`, `vix`) read fd 0 raw via the shared **`tkey.h`** (non-blocking + yield), and **`SYS_GETKEY` is pipe-aware** (`stdin_pipe_getchar` first) so keystrokes arrive through the pipe under mxterm. Full-screen apps (`maktop`, `clock`, `calc`) emit ANSI to stdout (T54); the kernel **cell-API → ANSI bridge** carries `sys_putch_at`-style output into the mxterm window. Shipped in `1b1dbb0` (+ T54).
