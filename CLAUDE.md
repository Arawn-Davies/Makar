...
- **Userspace**: Ring-3 protected mode via `iret`. ELF loader (`elf_exec`) with argc/argv. Syscalls: `SYS_EXIT`, `SYS_READ`, `SYS_WRITE` (fd 1 = VGA, fd 2 = VGA + COM1 serial), `SYS_OPEN`, `SYS_CLOSE`, `SYS_LSEEK`, `SYS_BRK`, `SYS_DEBUG`, `SYS_YIELD`, plus Makar extensions (200–215 - terminal/file ops + `SYS_WRITE_SERIAL` + `SYS_GETCWD`). Apps: `calc.elf`, `hello.elf`, `makbox.elf` (ls/cat/cp/mv/rm/rmdir/echo/pwd multicall), `vix.elf`, `diskinfo.elf`, `fdisk.elf`, `clock.elf`, `maktop.elf`, `kbtester.elf`.
