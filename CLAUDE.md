...
| `basic.elf` | C64-flavoured line-numbered **integer** BASIC.  `basic` (REPL) or `basic prog.bas` (load + RUN).  PRINT/LET/IF..THEN/GOTO/GOSUB/RETURN/FOR..NEXT/INPUT/REM/END/CLS + graphics PLOT/LINE/COLOR (via `SYS_DRAW_LINE`, native VESA res; "?NO GRAPHICS" in VGA text mode).  Integer-only because the kernel doesn't init/save the x87 FPU — fractional work uses fixed-point.  Ships `mandelbrot.bas` + `lines.bas` type-in samples in `/mnt/cdrom/apps` |
