...
| 12c | **COW `#PF` handler + CR0.WP** - on write-fault to COW-tagged user PTE: if refcount==1 just clear COW + set RW; else alloc fresh frame, memcpy, dec old refcount, install RW.  Fall through to existing panic for real faults.  Enables `CR0.WP` at paging_init so kernel writes also fault on RO user pages (required for SYS_READ-into-shared-buffer scenarios). | ✅ |
| 12d | **`SYS_FORK` (EAX=2)** - `task_fork()` clones a task slot, dups `fd_table` (deep-copies FILE buffers), inherits cwd/tty/user_brk, clones PD via 12b, resets sig handlers.  Child's kstack hand-built so its first `task_switch` ret lands in `fork_child_iret` (task_asm.S) — a 1:1 mirror of the isr_common_stub epilogue that pops a copy of parent's `registers_t` (EAX patched to 0) and `iret`s to ring 3.  `forktest.elf` exercises the path end-to-end. | ✅ |
| 12e | **Multi-page COW + exit-code coverage** - `forktest.elf` now seeds five independent BSS sentinels (one single-page, four explicitly 4 KiB-aligned), child writes all of them after fork (independent COW faults), parent verifies all five remain at original values; child exits with status=42 to confirm the kernel logs propagation.  `fork-cow` ui scenario asserts all of the above. | ✅ |
| 13a | **`SYS_EXECVE` (EAX=11)** - replaces caller's address space with a new ELF.  `elf_exec` extended to free the old user PD on success (no-op when old PD was `paging_kernel_pd()`, preserving the fresh-task path).  Syscall handler copies path + argv strings into kernel scratch before invoking elf_exec (user memory disappears at PD swap).  POSIX-compliant signal-handler reset on execve via `sig_task_init`.  `execvetest.elf` does fork→child-execve→parent-survives; `fork-execve` ui scenario asserts each leg. | ✅ |
| 13b | **`SYS_WAIT4` (EAX=114) + zombie state** - new `TASK_ZOMBIE` lifecycle between RUNNING and DEAD; child holds its slot + `exit_status` for the parent to read.  `task_t` gains `parent_pid` + `exit_status`; `task_fork` and `task_create` set parent_pid; `task_exit` decides ZOMBIE vs DEAD based on whether parent is a live ring-3 task (kernel-internal tasks go straight to DEAD so kernel shell children don't tie up pool slots).  Orphan zombies are auto-reaped by parent's `task_exit`.  `forktest.elf` + `execvetest.elf` now use `sys_wait4` instead of busy-yielding; both observe the child's exit status round-trip through wait4. | ✅ |
| 13c | **Userland shell (initial)** - lift `shell_run` into a ring-3 ELF (`sh.elf` or similar) that uses fork+execve+wait4 to spawn applets.  Today's `shell_cmd_*.c` builtins migrate gradually; kernel keeps its in-tree shell as the fallback until the userland one is fully self-hosting.  Blocker: streaming I/O (real `sys_read` for files, not the eager-buffer model) before pipelines + redirection become useful. | ⏭ |
