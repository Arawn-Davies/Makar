...
1. `terminal_initialize` → `init_serial(COM1)` → `init_descriptor_tables` (GDT+IDT) → `fpu_init` (x87 armed: CR0.EM=0 + `fninit` + CR4.OSFXSR; per-task `fxsave`/`fxrstor` on every context switch)
