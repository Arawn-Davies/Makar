...
echo "# \$DIR/makar.kernel.tcc.  Copy to /boot/makar.kernel and reboot to test."
echo "#"
echo "# Scratch dir: tries /bin first (persistent on HDD installs; created"
echo "# by the installer), falls back to /tmp on read-only-rootfs boots"
echo "# (live CD).  tmpfs now supports nested paths so either works."
echo ""
echo "echo rebuild-kernel: BEGIN"
echo "mkdir /bin/ktcc"
echo "if [ \$? -eq 0 ]; then DIR=/bin; else DIR=/tmp; mkdir /tmp/ktcc; fi"
echo "echo rebuild-kernel: scratch \$DIR/ktcc"
echo "FAILED=0"
echo ""
echo "# _kernel_end sentinel is shipped at /apps/kend.S (kernel sh has no > redir)."
echo "exec /apps/tcc.elf -c /apps/kend.S -o \$DIR/ktcc/kend.o"
...
exec /apps/tcc.elf -ffreestanding -D__is_kernel -DDEV_BUILD -DMAKAR_IN_OS_BUILD=1 -I/usr/include/kernel-build -c $in_os -o \$DIR/ktcc/$nn.o
...
# Single direct link -- no intermediate archive.  Same recipe the host
# build uses (build-kernel-tcc.sh's phase 1).  Avoids TCC's tcc_tool_ar
# which produced a libk.a whose symbol index the linker couldn't see
# (kernel_main et al. came back undefined).
printf 'exec /apps/tcc.elf -nostdlib -nostdinc -static -Wl,-Ttext=0x100000 -Wl,-section-alignment=0x1000 -o $DIR/makar.kernel.tcc' >> "$OUT"
N=0
for f in $KFILES; do
    N=$((N + 1))
    nn=$(printf '%03d' "$N")
    printf ' $DIR/ktcc/%s.o' "$nn" >> "$OUT"
done
printf ' /usr/lib/tcc/libtcc1.a $DIR/ktcc/kend.o\n' >> "$OUT"
echo "if [ \$? -ne 0 ]; then echo rebuild-kernel: FAIL link; FAILED=1; fi"
echo ""
echo "if [ \$FAILED -eq 0 ]; then"
echo "  echo REBUILD-KERNEL: ALL PASS"
echo "  echo rebuild-kernel: cp \$DIR/makar.kernel.tcc /boot/makar.kernel and reboot"
echo "else"
