...
echo "# /bin/makar.kernel.tcc.  Copy to /boot/makar.kernel and reboot to test."
echo "# Output dir /bin lives on the rootfs (created by the installer);"
echo "# tmpfs is flat so /tmp can't host the per-source .o tree."
echo ""
...
echo "exec /apps/tcc.elf -c /apps/kend.S -o /bin/ktcc/kend.o"
...
exec /apps/tcc.elf -ffreestanding -D__is_kernel -DDEV_BUILD -DMAKAR_IN_OS_BUILD=1 -I/usr/include/kernel-build -c $in_os -o /bin/ktcc/$nn.o
...
for f in $KFILES; do
    ...
    [ "$N" -eq 1 ] && continue
    nn=$(printf '%03d' "$N")
    echo "exec /apps/tcc.elf -ar rcs /bin/ktcc/libk.a /bin/ktcc/$nn.o" >> "$OUT"
done

{
echo ""
echo "echo rebuild-kernel: link"
echo "exec /apps/tcc.elf -nostdlib -nostdinc -static -Wl,-Ttext=0x100000 -Wl,-section-alignment=0x1000 -o /bin/makar.kernel.tcc /bin/ktcc/001.o -Wl,--whole-archive /bin/ktcc/libk.a -Wl,--no-whole-archive /usr/lib/tcc/libtcc1.a /bin/ktcc/kend.o"
echo "if [ \$? -ne 0 ]; then echo rebuild-kernel: FAIL link; FAILED=1; fi"
echo ""
echo "if [ \$FAILED -eq 0 ]; then"
echo "  echo REBUILD-KERNEL: ALL PASS"
echo "  echo rebuild-kernel: cp /bin/makar.kernel.tcc /boot/makar.kernel and reboot"
echo "else"
