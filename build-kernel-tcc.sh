...
echo "# rebuild-kernel.sh -- GENERATED by build-kernel-tcc.sh.  Do not edit." > "$OUT"
echo "# Rebuilds the kernel inside Makar via /apps/tcc.elf, then writes" >> "$OUT"
echo "# /tmp/makar.kernel.tcc.  Copy to /boot/makar.kernel and reboot to test." >> "$OUT"
echo "" >> "$OUT"
echo "echo \"rebuild-kernel: BEGIN\"" >> "$OUT"
echo "mkdir /tmp/ktcc" >> "$OUT"
echo "" >> "$OUT"
echo "# _kernel_end sentinel" >> "$OUT"
echo "echo \".section .bss\"        > /tmp/ktcc/kend.S" >> "$OUT"
echo "echo \".global _kernel_end\" >> /tmp/ktcc/kend.S" >> "$OUT"
echo "echo \"_kernel_end:\"        >> /tmp/ktcc/kend.S" >> "$OUT"
echo "echo \".byte 0\"             >> /tmp/ktcc/kend.S" >> "$OUT"
echo "exec /apps/tcc.elf -c /tmp/ktcc/kend.S -o /tmp/ktcc/kend.o" >> "$OUT"
echo "if [ \$? -ne 0 ]; then echo \"rebuild-kernel: FAIL kend\"; exit 1; fi" >> "$OUT"
echo "" >> "$OUT"

# Per-file compile blocks (in-OS path uses /src not src; /usr/include/tcc for stubs)
N=0
for f in $KFILES; do
    N=$((N + 1))
    nn=$(printf '%03d' "$N")
...
if [ \$? -ne 0 ]; then echo "rebuild-kernel: FAIL $f"; exit 1; fi
EOF
done

# Build the archive incrementally (SHELL_MAX_ARGS=16 forbids passing all
# objects to the linker in one call).  Skip 001 (boot.o) -- linked separately.
echo "" >> "$OUT"
echo "echo \"rebuild-kernel: archive\"" >> "$OUT"
echo "rm /tmp/ktcc/libk.a" >> "$OUT"
N=0
for f in $KFILES; do
    N=$((N + 1))
    [ "$N" -eq 1 ] && continue   # boot.o linked explicitly first
    nn=$(printf '%03d' "$N")
    echo "exec /apps/tcc.elf -ar rcs /tmp/ktcc/libk.a /tmp/ktcc/$nn.o" >> "$OUT"
done

# Final link: boot.o first, then archive (whole), then libtcc1, then kend last.
echo "" >> "$OUT"
echo "echo \"rebuild-kernel: link\"" >> "$OUT"
echo "exec /apps/tcc.elf -nostdlib -nostdinc -static -Wl,-Ttext=0x100000 -Wl,-section-alignment=0x1000 -o /tmp/makar.kernel.tcc /tmp/ktcc/001.o -Wl,--whole-archive /tmp/ktcc/libk.a -Wl,--no-whole-archive /usr/lib/tcc/libtcc1.a /tmp/ktcc/kend.o" >> "$OUT"
echo "if [ \$? -ne 0 ]; then echo \"rebuild-kernel: FAIL link\"; exit 1; fi" >> "$OUT"
echo "" >> "$OUT"
echo "echo \"REBUILD-KERNEL: ALL PASS\"" >> "$OUT"
echo "echo \"==> /tmp/makar.kernel.tcc ready.  cp /tmp/makar.kernel.tcc /boot/makar.kernel; then reboot\"" >> "$OUT"
