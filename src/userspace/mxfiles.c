...
static void open_sel(void){
    ...
    if(br_sel_isdir(&g_files)){ br_enter_sel(&g_files); g_loadid++; }
    /* a regular file: navigation-only (the Editor opens files via its dialog). */
}
...
static int icon_view(ui_ctx*u, gfx_surface*s, int x,int y,int w,int h){
    int cw=92, ch=72, cols=w/cw; if(cols<1)cols=1;
    ...
    for(int i=0;i<g_files.n;i++){
        ...
        if(i==g_files.sel) gfx_round(s,cx+2,cy+2,cw-4,ch-4,UI_COL_BTN_ACT,UI_COL_FIELD);
        else if(hot)       gfx_round(s,cx+2,cy+2,cw-4,ch-4,UI_COL_BTN_HOT,UI_COL_FIELD);
        int gx=cx+(cw-30)/2, gy=cy+8;
        if(g_files.type[i]==DT_DIR) draw_folder(s,gx,gy); else draw_file(s,gx,gy);
        char nm[18]; scpy(nm,g_files.name[i],sizeof nm);
        gfx_str_clip(s,cx+(cw-gfx_text_w(nm))/2,cy+ch-18,nm,UI_COL_TEXT,cx+cw-2);
