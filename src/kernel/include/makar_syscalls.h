...
/* BSD sockets (subset) -- TCP/IPv4 over the in-kernel lwIP stack.  The kernel
 * owns the TCP/IP stack; userspace owns the protocols above it (HTTP, TLS) and
 * reaches the wire through these.  read()/write()/close() operate on the socket
 * fd directly (Linux-uniform), so only create + connect + DNS need new numbers.
 * DNS resolution lives in the kernel (lwIP) -- no userspace resolver yet -- so
 * resolve is a syscall, not a libc routine. */
#define SYS_SOCKET          280 /* int socket(int domain, int type, int proto) -> fd */
