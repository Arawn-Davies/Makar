...
- **img_bmp** — a shared uncompressed-BMP decoder (`bmp_decode`/`bmp_load`) used
  by `mximg` and by the window manager for desktop icons.

**Desktop icons** are real BMP tiles on an XFCE-style asset path:
`/usr/share/icons/makar/<name>.bmp`, loaded once at WM startup (`load_icon_assets`)
and blitted in `draw_icons`. If an asset is missing (e.g. a minimal live boot) the
procedural `icon_glyph()` draws instead, so the desktop always has icons. The
tiles are generated by `tools/mkicons.py` (committed, so the build has no
dependency on it). DOOM IWADs likewise live on a normal path,
