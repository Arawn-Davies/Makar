...
| **Multi-TTY** | 4 preemptive shell tasks `shell0`–`shell3`, **Alt+F1–F4** to switch. Per-TTY `vt_buf_t` backing grid; FB painted only when focused; **makmux** multiplexer's status bar (`Makar  VT1 VT2 VT3 VT4`) at the bottom row. Framebuffer syscalls are focus-gated so a backgrounded fullscreen app stays isolated to its VT. |
