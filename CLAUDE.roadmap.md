...
| 28 | **POSIX pipes + dup/dup2 + streaming fd refactor** — kernel-side primitives that the userland shell needs for `|`, `>`, `<`, `2>&1`.  Sub-slices: (a) `open_file_t` ref-counted backing object so `fd_table_clone` (fork) bumps refcounts instead of deep-copying buffers; (b) `SYS_DUP` (EAX=41) + `SYS_DUP2` (EAX=63) with standard Linux i386 semantics (`dup2(oldfd, newfd)` closes newfd first); (c) `SYS_PIPE` (EAX=42) + new `kernel/pipe.c` with a 4 KiB ring-buffered pipe, `FD_KIND_PIPE_R`/`FD_KIND_PIPE_W`, yield-block on empty/full, EOF when all writers closed; (d) `SYS_READDIR` ring-3 hookup -- already shipped (`sys_readdir` at `src/userspace/syscall.h:390`, consumed by `alloctest.c`). | 🟡 (a-c) ⏭ ; (d) ✅ |
