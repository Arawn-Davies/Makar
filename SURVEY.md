...
- Full-screen links/w3m-style terminal browser (cell API): fetches HTTP/HTTPS via `web_fetch` (userspace `web.c`/`tls.c`), reflows HTML into a scrollable page with a top menu bar, a navigable highlighted **focus ring** (links **and** `<input>` form fields, walked by Tab/↑/↓/`n`/`p`), and a status line
- **Form typing:** text inputs are editable boxes with a full caret; **Enter submits** the enclosing `<form>` (`action?name=value&…`, urlencoded, GET-style); hidden inputs ride along. Single-letter commands are suppressed while a field is focused
- `g` go-URL, `b`/← back, `r` reload, Enter/→ follow a link, PgUp/PgDn/Space/`j`/`k` scroll, F9/`m` File menu, `q` quit. `linx [--dump] <url|file>`; opens local/`file://` paths + an `about:linx` start page. Works in a text VT and under mxterm (raw keys via `tkey.h`)
...
- `gui.elf uitest` / `fstest` / `desktest` are headless, framebuffer-free self-tests (widgets / VFS file-browser / desktop icon-grid+sort+`~/.mxrc` persistence; markers `GUI-UITEST`/`GUI-FSTEST`/`GUI-DESKTEST`). They form the **`gui-smoke`** suite (`/src/userspace/gui-smoke.sh`, marker `GUI-SMOKE`) — its own logical test section, run by `./run.sh guismoke` and the `gui-headless` CI job (separate from `ktest` and the `guitest` boot-screendump job). Built-in graphical login (`gui login`). See `docs/gui.md`, `docs/testing.md`.
...
- Shared client libs: `gui_gfx` (drawing), `gui_ui` (immediate-mode widgets), `gui_browser` (FS model + file dialog), `makx` (protocol client), `img_bmp` (shared BMP loader), `img_png` (shared from-scratch PNG decoder), `img_ico` (shared ICO decoder), `img_jpg` (shared baseline-JPEG decoder) — all three used by `mximg` + the `wm` desktop-icon loader.
- Desktop icons are XFCE-style `.desktop` shortcuts (`Name`/`Icon`/`Exec` + `X-Makar-*`): `wm` scans `/usr/share/shortcuts` + the `~/.shortcuts` user overlay. Windows-style layout — sorted **case-folded by Name** (filenames carry no numeric prefix), arranged in a **column-major height-derived grid** (never clipped off the bottom; `icon_grid_pos`+`icon_clamp`). Draggable + selectable; drag-drop and **Auto Arrange** persist positions per user in `~/.mxrc` (`IconPos.<Name>=x,y`), not in the `.desktop`. `.ico` artwork generated by `tools/bmp2ico.py`.
- macOS-style desktop chrome: menu-bar **Edit** (Cut/Copy/Paste) + **View** (Auto Arrange) dropdowns and an empty-desktop **right-click context menu** with the same actions. Cut/Copy/Paste inject `^X`/`^C`/`^V` into the focused client (the app's own clipboard runs; no undo/redo); Auto Arrange re-grids + persists every icon. Shared popup module mirrors the dock tray-menu pattern.
