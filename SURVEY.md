...
- Desktop icons are XFCE-style `.desktop` shortcuts (`Name`/`Icon`/`Exec` + `X-Makar-*`): `wm` scans `/usr/share/shortcuts` + the `~/.shortcuts` user overlay, icons are draggable + selectable (drag persists `X-Makar-IconX/Y` back to the file). `.ico` artwork generated by `tools/bmp2ico.py`.
