...
- Shared client libs: `gui_gfx` (drawing), `gui_ui` (immediate-mode widgets), `gui_browser` (FS model + file dialog), `makx` (protocol client), `img_bmp` (shared BMP loader), `img_png` (shared from-scratch PNG decoder), `img_ico` (shared ICO decoder) — all three used by `mximg` + the `wm` desktop-icon loader.
